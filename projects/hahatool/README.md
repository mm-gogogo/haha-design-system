...
### 6.2 Showcase v2 富化区块（preview.html 已落地）

preview.html 已从「色板 + 组件清单」升级为**有图、有布局、信息饱满**的作品级展示页。新增四大区块，全部走 `--ds-*` / `rgb(var(--brand-*))` / `--chart-*`，随 4 主题色与明暗一起换肤；图片一律 `loading="lazy"` + 有意义 `alt`，仅引用 `_assets/**` 与 `_fonts/fonts.css`。

- **Hero 首屏**：顶部分栏 hero（左文案 + CTA + 关键指标 mono / 右 `enterprise-hero.jpg` 图 + 渐变 scrim + 浮卡）。消灭首屏留白，正式可信。
- **图片用法 Image Usage**：
  - 宽高比框 16:9 / 4:3 / 1:1 / 3:4（统一 `aspect-ratio` + `object-fit:cover`，圆角 md）。
  - 媒体卡（工具卡配图）：图在上 + 标题 / 简介 / 数据脚，≥3 张成网格；带推广 / 定价徽章。
  - 截图缩略：浏览器 chrome 小窗 + 16:10 缩略图。
  - 图上叠字 Overlay：图 + 渐变 scrim + 标题，文字对比保证 AA。
  - 图文混排：大图 + 文字段落并排（编辑式介绍），窄屏堆叠。
  - 头像用法：尺寸阶 24 / 32 / 40 / 48、堆叠头像组（带描边分隔 + `+N`）、用户行（头像 + 名 + 签名）。
- **图标用法 Icon Usage**：图标集网格（≥12 个内联 SVG，带名）；尺寸阶 16 / 20 / 24 / 32 基线对齐；线性 vs 实心、单色 vs 语义双色对比；场景演示（图标按钮带 `aria-label`、按钮前后置图标、输入框前后置图标、列表前导图标、语义状态图标、带数字徽标 `tabular-nums`）。
- **桌面布局 Desktop Layouts ×2**（带浏览器 chrome 的 frame）：
  1. **AI 工具导航首页**：左侧分类导航 + 顶部搜索 + 标签筛选 chips + 工具卡网格（配图）+ 右栏流量榜单。
  2. **工具详情**：左主区（工具头 + 近 6 月流量柱状 + 用户评论）+ 右栏（能力雷达 + 信息列表）。雷达 / 柱状全手写 SVG，走 `--chart-*` + brand。
- **移动布局 Mobile Layouts ×2**（~375px 手机框，并排）：
  1. **App 首页**：顶栏（折叠汉堡）+ 搜索 + 工具卡 + 整宽主操作 + Top 榜单。
  2. **工具详情 / 个人页**：大图 header（叠 logo + 名）+ 操作按钮 + 关键指标 + 最新评论。

> 第二个能力雷达（详情页）复用 `paintRadar(svg, withB)`，详情用单系列、PK 用双系列；明暗 / 换肤时一并重绘。
