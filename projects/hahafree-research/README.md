...
## 9b. 展示页 v2 · 图片 / 图标 / 布局规范

`preview.html` 升级为作品级展示页：顶部 hero + 图片用法 + 图标用法 + 桌面/移动多布局，填满留白、更正式。素材一律引本地 `../../_assets/**`，`loading="lazy"` + 有意义 `alt`，除 `fonts.css` 外无任何外链。

### Hero 首屏

分栏式：左文案（eyebrow 徽章 + 30px 标题 + lede + 双 CTA + 四项 hstats）+ 右 `bside-hero.jpg`（`object-fit:cover`），右图叠**渐变 scrim**（向左淡出到面色，保证左文案区与底图不冲突）+ 右下「实时迭代」状态徽标。窄屏图移到顶部、scrim 转纵向。占满首屏，消灭顶部留白。

### 图片用法 Image Usage

数据台图片处理身份：**规整框 + 统一 6px 小圆角 + 1px 描边 + 弱阴影**（不放大圆角、不堆重影），覆盖：

- **宽高比框**：16:9 / 4:3 / 1:1 / 3:4，统一 `aspect-ratio` + `object-fit:cover`。
- **媒体卡 Media Card**：截图 + 赛道 pin + 标题/简介 + 状态点/浏览数副行，≥3 张成网格，hover 描边转玫瑰。
- **截图画廊**：6 图等高 1:1 网格，hover 轻微放大（`prefers-reduced-motion` 下停用）。
- **图文混排**：左大图 + 右「质量分引擎」介绍 + 冷青对勾要点列表。
- **图上叠字 Overlay**：底图 + 向左压暗的 `--ds-fg` 渐变，标题/正文反白 AA 达标。
- **头像用法**：尺寸阶 24/32/40/48、堆叠 `av-stack`（+N 计数）、用户行（头像 + 名 + mono 副信息）。

### 图标用法 Icon Usage

统一内联 SVG（`viewBox="0 0 24 24"`、线宽 1.8、`currentColor`，零 emoji）：

- **图标集**：16 个带名图标网格（analytics / chart-bar / search / filter / export / star / refresh / table / clock / add / trash / layout / activity / settings / watchlist / signal）。
- **尺寸阶**：16 / 20 / 24 / 32，基线对齐（线宽随尺寸微调保持视重）。
- **线性 vs 实心 / 单色 vs 双色**：线性为默认，实心仅用于选中/状态，双色用 `color-mix` 浅填 + 描边。
- **在场景里**：图标按钮（带 `aria-label`）、按钮内前/后置图标、输入框前置图标、列表项前导图标、语义状态图标、带数字徽标的通知图标。

### 桌面布局 Desktop Layouts（≥2）

均包在带 `dots + 地址栏 + 标题` 的浏览器 chrome frame 内：

1. **数据情报台仪表盘**：左侧导航轨（当前项左 3px 玫瑰条 + 浅玫瑰底）+ 顶部操作行 + 4 张 KPI + 手写图表（近 7 轮柱状 `--chart-*` + 赛道占比 SVG 环形图 + 图例）+ 赛道 Top 密集小表。
2. **项目列表-详情**：左列表（搜索 + 带缩略图/ID/质量分的行、当前行浅玫瑰）+ 右详情（大图 header + 标题/元信息 + 4 格指标 + 正文简介 + 加入 watchlist）。

### 移动布局 Mobile Layouts（≥3）

≈375px 手机框（听筒状态栏 + 底部说明条）：

1. **M1 情报台首页**：面包屑 + 2 列 KPI + 精简项目列表 + 整宽主操作。
2. **M2 媒体 feed**：图文卡流（截图 + 赛道 Tag + 分数）+ 底部 4 项图标标签栏（当前项玫瑰）。
3. **M3 项目详情**：大图 header + 反白叠字 + 三格统计 + 简介 + 整宽 CTA。

> 与基底气质一致：B 端中性灰玫瑰主色 + 冷青分析、高密度、小圆角、弱阴影；图片走规整框，不引入装饰性强冲击。
