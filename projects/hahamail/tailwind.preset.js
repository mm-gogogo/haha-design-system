/**
 * hahamail · Tailwind Preset  ——  基于「B 端风」+ hahamail 定制
 * ------------------------------------------------------------
 * 基建/邮件管理气质（沉稳靛蓝 · 高密度 · 可观测后台）。
 * 消费 projects/hahamail/tokens.css 中的 CSS 变量（--ds-*），不硬编码取值。
 *
 * 用法：
 *   1. 在入口引入本 kit 的 tokens.css（提供 --ds-* 变量与亮/暗主题）。
 *   2. tailwind.config.js: { presets: [require('./projects/hahamail/tailwind.preset.js')] }
 *   3. 暗色：<html data-theme="dark"> 或 class="dark"（运维默认暗色）。
 *
 * 颜色直接透传 CSS 变量；需要 alpha 时可改用 rgb(var(--…)/<alpha-value>) 形式。
 */
module.exports = {
