...
## 6.5 图片与图标用法（Showcase v2）

`preview.html` 已升级为作品级展示页：顶部 hero + 图片用法 + 图标用法 + 桌面布局 ×3 + 移动布局 ×2。图片与图标如何「落进新拟物身份」是本 Kit 的关键，规则如下。

### 图片处理（身份：嵌凹槽 / 托凸卡，勿用裸图）

新拟物的图片**绝不裸放**——一律嵌进 **inset 凹槽**（图像像被「压」进薄荷软塑料）或托在**凸起卡**上，统一大圆角 + 极轻凸阴影，与控件同一种材质语言。

- **宽高比框**：统一用 `aspect-ratio` + `object-fit:cover`，覆盖 16:9 / 4:3 / 1:1 / 3:4；外层套 `--ds-bg-sunken` + `--ds-shadow-inset` 凹槽，图本身圆角 `--ds-radius-sm` + `--ds-shadow-sm`。
- **媒体卡 Media Card**：图嵌卡内 inset 小凹槽（`--ds-shadow-inset-sm`）+ 标题 / 正文 / 操作；≥3 张成网格，用于「app 截图 / 磁盘可视化 / 功能介绍」。
- **图片网格 / 画廊**：等高网格（含一个 `span 2` 宽幅），每图 `--ds-shadow-sm` 微凸。
- **图文混排**：左图（inset 凹槽）+ 右文字段落 + 带图标的特性列表，用于「磁盘可视化」这类产品介绍。
- **图上叠字 Overlay**：图 + **左向渐变 scrim**（薄荷墨绿 `rgba(20,40,36,…)`）压暗，保证白字 ≥4.5:1 AA。
- **背景大图区**：full-bleed 背景 + 上深下浅遮罩 + 白字内容与统计。
- **头像**：尺寸阶 24 / 32 / 40 / 48（圆形 + `--ds-shadow-sm`），堆叠 `avatar-stack`（3px `--ds-bg` 描边相叠 + 「+N」凹陷计数），用户行（头像 + 名 + 签名）。
- **媒体素材**：app 截图 / 磁盘可视化 / 媒体一律放 inset 凹槽或凸卡，不破坏「同底软质」气质。
- 全部图片 `loading="lazy"` + 有意义 `alt`；素材来自 `../../_assets/`（hero 用 `gen/neumorph-hero.jpg`，配图用 `photos/*`、`avatars/*`、`gen/abstract-wave.jpg`），**无任何外网图**。

### 图标处理（身份：内联 SVG + 轻浮雕底牌）

- 全部**内联 SVG**（零 emoji），`viewBox="0 0 24 24"`、`stroke="currentColor"`、线宽 ~1.8、圆头圆角；颜色继承上下文或用 `--ds-fg` / `--ds-primary-strong`，**务必 ≥4.5:1**。
- **图标底牌**：导航 / 列表 / 仪表的图标常坐在小 inset 凹槽（`--ds-bg-sunken` + `--ds-shadow-inset-sm`）或微凸方牌上，呼应新拟物「轻浮雕」。
- **尺寸阶**：16 / 20 / 24 / 32，基线对齐统一。
- **线性 vs 实心 / 单色 vs 双色**：默认线性单色；强调态可实心；状态图标（成功 / 警告 / 危险 / 信息）用语义色双色（浅底 `-bg` + 主色描边 + `-fg` 对勾 / 叉 / 感叹）。
- **在场景里**：纯图标按钮（带 `aria-label`）、按钮内前 / 后置图标、输入框前导图标、状态徽章图标、带数字角标的徽标图标（角标用 `--ds-danger` + 白字 + `tabular-nums`）。

### preview.html 区块总览（v2）

| 区块 | 内容 |
|---|---|
| Hero 首屏 | 图文分栏，hero 图嵌凹槽 + 标题 / CTA / 关键指标，消灭首屏留白 |
| 图片用法 | 宽高比框、媒体卡、图文混排、Overlay、头像用法、画廊、背景大图 |
| 图标用法 | 18 图标集、尺寸阶、线性/实心/双色、场景中（图标按钮 / 输入 / 角标） |
| 桌面布局 ×3 | ① 清理主窗（左导航 + 概览仪表 + 扫描结果）② 磁盘分析页（树状图 + 大文件列表）③ 官网落地页（hero 图文 + 三栏特性） |
| 移动布局 ×2 | ① App 首页（大主操作 + feed + 底部标签栏）② 详情 / 设置页（大图 header + 开关项） |
| 组件 | 原有 7+ 组件（按钮 / 列表 / 仪表 / Modal / Dropdown / Tooltip / 分页 / Toast / Stepper / 面包屑 / Alert / 色板 / 字体）全部保留 |

> 桌面小样放进带窗口 chrome（红黄绿圆点 + 名称 / 地址栏）的 `.frame-win`；移动小样放进 ~375px `.phone-frame`（听筒 + 状态栏意象）。布局全部用真实结构与可信文案，非占位块。
