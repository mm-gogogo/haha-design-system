...
>
> **怎么用（给 AI/agent）**：把本文件整篇读入即可。它完整定义了 `--ds-*` Token 契约 + 49 套风格的关键取值 + 6 个项目定制 + 接入方式。
...
## 4. 49 套通用风格（关键取值；完整值见各 tokens.css）
...
### 48 漫画 · Manga / Ink & Screentone — `/styles/48-manga/`
日式漫画黑墨语言（light-first）：单行本纸面 + 浓黑粗描分镜（部分斜切分镜）+ 灰阶网点 screentone 阴影 + 集中線速度线 + 唯一一个高声量点色「朱红」+ 圆形红印（hanko）+ 拟声词 sfx 标签；高对比、图像化、叙事感。亮（`:root` 微暖单行本纸，非饱和米）primary 朱红 `#cf2417` / accent 墨黑 `#141414` / bg `#f2f1eb`·elevated `#fbfaf6` / fg 墨 `#141414`·muted `#4a4a48` / border-strong 墨黑｜暗（反相夜场跨页）primary 亮朱红 `#ff4436`(深字) / accent 纸白 `#f2f0e8` / bg `#0d0d0d`·elevated `#1c1c1c` / fg `#f2f0e8`｜0·2·4（方形分镜）｜Noto Serif 重磅戏剧标题/拟声词 + Inter 黑体正文 + JetBrains Mono 分镜号/sfx｜分镜栅格 + 网点 + 集中線 + 朱红点睛 + 圆印；墨黑 + 唯一朱红；虚构「墨线漫画 Inkline Comics」。与 28 波普（彩色 Ben-Day 网点）、44 极简黑白（无插画无网点）、38 黑色电影（摄影写实）刻意区分（黑墨网点速度线漫画语言）。

### 49 复古音响 · Hi-Fi / Vintage Audio — `/styles/49-hifi/`
70/80 年代立体声接收机/功放控制面语言（light-first）：香槟拉丝铝面板 + 琥珀背光 VU 指针表 + 滚花旋钮 + 线性推子 + 分段 LED 电平条 + 胡桃木侧板 + 调频调谐刻度 + POWER/STEREO/PEAK 指示灯；温暖、模拟、精密，非数字辉光非塑料光泽。亮（`:root` 香槟拉丝面板）primary 琥珀铜 `#a8621a` / accent 青绿 `#1f6f6a` / bg `#e7e5df`·elevated `#f4f2ec` / fg `#26241f`·muted `#54514a`｜暗（胡桃木 + 石墨夜间监听台）primary 琥珀 `#e0962f`(深字) / accent `#4fb8ad` / bg `#1a1816`·elevated `#2a2622` / fg `#ece6da`｜3·6·10 + 圆旋钮｜Inter 标签 + JetBrains Mono 频率/dB/电平读数｜VU 指针表 + 滚花旋钮 + 推子 + 分段 LED + 胡桃木；琥珀 + 青绿；虚构「声谷音响 Vale Audio」接收机。与 32 拟物写实（通用皮革凝胶玻璃）、40 酸性镀铬（光泽熔铬）、08 新拟物（柔 UI 凸凹）刻意区分（音响器材 VU 表/旋钮/木纹专属母题）。
