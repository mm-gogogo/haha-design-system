/**
 * Luxury · 奢侈高端 Tailwind 预设
 * 全部消费 tokens.css 里的 CSS 变量（--ds-*），亮/暗主题自动跟随。
 * 用法：
 *   // tailwind.config.js
 *   module.exports = { presets: [require('./styles/11-luxury/tailwind.preset.js')] }
 *   // 并在入口引入 tokens.css
 *   import './styles/11-luxury/tokens.css'
 *
 * 提示：本风格暗为主。标题/display 用 font-serif（Noto Serif 细字重、字距讲究），
 *   正文/控件用 font-sans（Inter）。留白是语言——多用大 spacing，少塞内容。
 *   图片 full-bleed + 细金描边（border-color: var(--ds-border)），圆角小，阴影克制。
 */
module.exports = {
  theme: {
    extend: {
  ...
  // 本风格暗为主：默认即暗（:root）；亮模式用 [data-theme="light"] 触发。
