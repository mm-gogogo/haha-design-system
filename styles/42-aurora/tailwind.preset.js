/**
 * Aurora · 极光渐变 (Gradient Mesh) Tailwind 预设
 * 全部消费 tokens.css 里的 CSS 变量（--ds-*），dark-first，亮色由 [data-theme="light"] 或 .light 驱动。
 *
 * 用法：
 *   // tailwind.config.js
 *   module.exports = { presets: [require('./styles/42-aurora/tailwind.preset.js')] }
 *   // 并在入口引入 tokens.css
 *   import './styles/42-aurora/tokens.css'
 *
 *   <div class="bg-bg-elevated border border-border rounded-md shadow-md text-fg">
 *     Solid Aurora card over a decorative gradient mesh
 *   </div>
 *
 * 注意：gradient mesh 是品牌表面/装饰层，不承载正文小字；文字保持 solid color。
 */
module.exports = {
  theme: {
    extend: {
  ...
  // 亮色用 [data-theme="light"] 或 .light 驱动；:root 默认就是暗色。
