/**
 * Orbit HUD · 航天终端 / 科幻 HUD — Tailwind Preset
 * ---------------------------------------------------------------------------
 * 全量映射 --ds-* CSS 变量。先在入口引入 tokens.css（提供 :root 暗色基底 +
 * [data-theme="light"]/.light 亮色覆盖），再在 tailwind.config.js 里：
 *   presets: [require('./tailwind.preset.js')]
 *
 * ⚠️ 本风格暗色优先：:root 即暗色，亮色靠 [data-theme="light"] / .light 覆盖。
 *    因此这里把 darkMode 视为「默认」，并提供 light: 变体方便写亮色专属样式。
 *
 * 航天 HUD 气质要点：
 *   - radius 偏锐利（sm4/md8/lg12），仪表面板硬朗。
 *   - shadow 叠加一束克制的离子蓝辉光（见 tokens.css）。
 *   - boxShadow.glow / glow-accent 提供离子蓝发光工具类。
 *   - 动效利落 150–250ms。
 */
module.exports = {
