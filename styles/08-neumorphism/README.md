...
## 13. 图片 / 图标 / 布局展示（preview.html）

`preview.html` 除色板 / 字体 / 19 个组件外，还含一套作品级展示区块，演示本风格在真实界面中的用法。所有图片来自 `../../_assets/`（本地素材，`loading="lazy"` + 有意义 `alt`），除 `fonts.css` 外无任何外链。

### Hero 首屏
顶部为凸起大卡（`--ds-shadow-lg`）分栏：左侧标题 / 副标题 / CTA / 关键指标，右侧 hero 图嵌入 `inset` 凹槽（`--ds-bg-sunken` + `--ds-shadow-inset`），并叠一张「对比度」浮起小卡——一凹一凸即新拟物的图片语言。

### 图片用法 Image Usage
- **宽高比框**：16:9 / 4:3 / 1:1 / 3:4，统一 `aspect-ratio` + `object-fit:cover`，全部嵌进 `inset` 凹槽，圆角走 `--ds-radius-sm/md`。
- **媒体卡**：凸起卡内放 inset 图槽 + 标题正文 + 操作（≥3 张网格）。
- **画廊**：等高凹槽网格（≥6 图），含 `g-wide` / `g-tall` 跨格。
- **图文混排**：大图（inset 槽）+ 段落 + 勾选式特性列表并排。
- **图上叠字 Overlay**：图 + 暗向渐变 `scrim` + 白色标题（scrim 始终深色，白字两主题下均 ≥4.5:1）。
- **头像用法**：尺寸阶 24/32/40/48（头像带 `--ds-shadow-sm` 软凸 + 内边距软衬）、堆叠头像组、用户行。
- **背景大图区**：full-bleed 图 + 渐变遮罩 + 内容 + 半透明玻璃按钮。

> **图片处理身份**：本风格图片一律放进 `inset` 凹槽（像被压进软材质）或托在大圆角凸起卡上，配 `--ds-shadow-sm` 柔影——**不**用硬边、不用粗描边、不用尖角。

### 图标用法 Icon Usage
- **图标集**：≥12 个统一线性图标（24×24 viewBox、1.8 线宽、圆角端点），每个坐在 `inset` 小坑（`--ds-shadow-inset-sm`）里。
- **尺寸阶**：16 / 20 / 24 / 32，基线对齐（小尺寸适当加粗线宽补偿）。
- **线性 vs 实心 / 单色 vs 双色**对比。
- **在场景里**：图标按钮（有 `aria-label`）、按钮内前 / 后置图标、输入框前 / 后缀图标、语义状态徽章、带数字角标的通知图标。

### 桌面布局 Desktop Layouts（×3）
均放进带浏览器 chrome（红黄绿圆点 + 地址栏）的凸起 frame，内容区为 `inset` 大面：
1. **设备控制面板 / 仪表盘**：侧边导航（选中项凹陷被按入）+ 顶栏 + 3 个 KPI 凸卡 + inset 图表区（柱状）。
2. **音乐播放器**：左「正在播放」凸卡（inset 封面槽 + 进度条）+ 右播放列表（当前项凹陷）。
3. **账户设置页**：左侧设置导航 + 右用户信息 + 开关 / 下拉行。

### 移动布局 Mobile Layouts（×2）
约 375px 手机框，软阴影强度在窄屏**不缩小**：
1. **健康概览**：顶栏 + 数据凸卡 + 满宽主按钮 + 凹槽列表 + 底部标签栏（图标，当前项凹陷）。
2. **音乐播放器**：凸起方形封面 + 标题 + inset 进度条 + 播放控制簇。

> 所有展示区块仍只走 `var(--ds-*)` 变量，亮 / 暗双主题与响应式（960px / 640px 断点）下均正常；身份特征（同底卡 + 双向柔和阴影 / inset 凹陷 + 大圆角）贯穿始终，未被 over-round / ghost-card 抹平。
