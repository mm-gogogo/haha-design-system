/**
 * Synthwave / 蒸汽波 / 合成波（Outrun）— Tailwind Preset
 * ---------------------------------------------------------------------------
 * 全量映射 --ds-* CSS 变量。先在入口引入 tokens.css（提供 :root 暗色基底 +
 * [data-theme="light"]/.light 亮色覆盖），再在 tailwind.config.js 里：
 *   presets: [require('./tailwind.preset.js')]
 *
 * 本风格暗色优先：:root 即暗色，亮色靠 [data-theme="light"] / .light 覆盖。
 * 因此把 darkMode 视为「默认」，并提供 light: 变体方便写亮色专属样式。
 *
 * Synthwave 气质要点：
 *   - radius 极小（sm2 / md6 / lg10），贴近 arcade 面板与霓虹描边。
 *   - shadow 叠 magenta / cyan 辉光（见 tokens.css）。
 *   - boxShadow.glow / glow-accent / glow-warning 提供按钮与状态灯发光工具类。
 *   - display 可用 sans heavy + wide tracking；数据/代码走 mono；动效利落 150–260ms。
 */
module.exports = {
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        ...
        // Outrun glow utilities（按钮 hover / 状态灯 / 描边）
        glow: '0 0 20px -2px var(--ds-primary)',
        'glow-accent': '0 0 20px -2px var(--ds-accent)',
        'glow-warning': '0 0 20px -2px var(--ds-warning)',
      },
