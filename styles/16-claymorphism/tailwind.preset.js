/**
 * Claymorphism · 黏土 3D Tailwind 预设
 * 全部消费 tokens.css 里的 CSS 变量（--ds-*），所以亮/暗主题自动跟随。
 * 含黏土双阴影：shadow-sm/md/lg + clay-raised（胖凸起）/ clay-pressed（按下凹）/ clay-accent（主色凸起）。
 * 用法：
 *   // tailwind.config.js
 *   module.exports = { presets: [require('./styles/16-claymorphism/tailwind.preset.js')] }
 *   // 并在入口引入 tokens.css
 *   import './styles/16-claymorphism/tokens.css'
 *
 *   <button class="bg-primary text-primary-fg rounded-lg shadow-clay-accent
 *                  active:shadow-clay-pressed transition duration-fast ease-ds">捏一下</button>
 *   <div class="bg-bg-elevated rounded-lg shadow-clay-raised p-6">黏土卡片</div>
 *   <input class="bg-bg-sunken text-fg rounded-md shadow-clay-pressed" />
 */
module.exports = {
  theme: {
    extend: {
      ...
      boxShadow: {
        ...
        lg: 'var(--ds-shadow-lg)',
        // 黏土质感
        'clay-raised': 'var(--ds-clay-raised)',   // 胖嘟嘟外凸（卡片/按钮默认）
        'clay-pressed': 'var(--ds-clay-pressed)',  // 按进黏土（:active / 选中 / 输入槽）
        'clay-accent': 'var(--ds-clay-accent)',    // 落在主色上的凸起
        // 焦点环
